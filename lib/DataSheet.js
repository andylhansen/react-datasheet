...
var DataSheet = function (_PureComponent) {
  ...
  function DataSheet(props) {
    ...
    _this.onClickDrag = _this.onClickDrag.bind(_this);
    ...
    _this.isCopyTarget = _this.isCopyTarget.bind(_this);
    _this.isSelectionCorner = _this.isSelectionCorner.bind(_this);
    ...
    _this.getDragTargetRange = _this.getDragTargetRange.bind(_this);
  ...
  _createClass(DataSheet, [{
    ...
    key: 'handleCopyDrag',
    value: function handleCopyDrag() {
      var range = this.getDragTargetRange();
      if (!range) {
        return;
      }

      var targetStart = range.start,
          targetEnd = range.end;
      ...
      var leftX = Math.min(start.j, end.j);
      var rightX = Math.max(start.j, end.j);

      var topY = Math.min(start.i, end.i);
      var bottomY = Math.max(start.i, end.i);
      ...
      var cols = rightX - leftX + 1;
      var rows = bottomY - topY + 1;

      for (var i = targetStart.i; i <= targetEnd.i; i++) {
        for (var j = targetStart.j; j <= targetEnd.j; j++) {
          var cell = data[i] && data[i][j];
    ...
    key: 'onClickDrag',
    value: function onClickDrag(i, j) {
      var cell = this.props.data[i][j];
      if (!cell.readOnly) {
        this._setState({
          dragging: true,
          ...
          dragInitialSelection: {
            start: _extends({}, this.state.start),
            end: _extends({}, this.state.end)
          }
        });
    ...
    value: function onChange(row, col, value) {
      var _props6 = this.props,
          ...
          onCellsChanged = _props6.onCellsChanged,
          data = _props6.data;
    ...
    key: 'isSelectionCorner',
    value: function isSelectionCorner(i, j) {
      var _getState8 = this.getState(),
          start = _getState8.start,
          end = _getState8.end;

      var x = Math.max(start.j, end.j);
      var y = Math.max(start.i, end.i);
      return x === j && y === i;
    }
  }, {
    key: 'getDragTargetRange',
    value: function getDragTargetRange() {
      var _getState9 = this.getState(),
          start = _getState9.start,
          end = _getState9.end,
          _getState9$dragEnd = _getState9.dragEnd,
          i = _getState9$dragEnd.i,
          j = _getState9$dragEnd.j;
    ...
    key: 'isCopyTarget',
    value: function isCopyTarget(i, j) {
      if (!this.state.dragging) {
        return false;
      }
    ...
    value: function render() {
      ...
      var _props7 = this.props,
          SheetRenderer = _props7.sheetRenderer,
          RowRenderer = _props7.rowRenderer,
          cellRenderer = _props7.cellRenderer,
          dataRenderer = _props7.dataRenderer,
            ...
            return _react2.default.createElement(
              ...
              row.map(function (cell, j) {
                return _react2.default.createElement(_DataCell2.default, {
                  ...
                  row: i,
                  col: j,
                  canClickDrag: draggable && _this5.isSelectionCorner(i, j),
                  cell: cell,
                  forceEdit: forceEdit,
                  onClickDrag: _this5.onClickDrag,
                  ...
                  isCopyTarget: _this5.isCopyTarget(i, j),
